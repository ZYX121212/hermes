...
async fn chat_completions_handler(
    ...
    if req.model != "auto" {
        ...
        if req.stream {
            return handle_stream(adapter, prompt, &req.model, false, "").await;
        }
        return handle_non_stream(adapter, prompt, &req.model, false, "").await;
    }

    // Route via gateway pipeline
    let (output, reasoning) = state.gateway.route(&prompt, mode).await;
    ...
    match output {
        GatewayOutput::Single { model, prompt } => {
            ...
            if req.stream {
                return handle_stream(adapter, prompt, &routed_model, shg_triggered, &reasoning).await;
            }
            handle_non_stream(adapter, prompt, &routed_model, shg_triggered, &reasoning).await
        }
        GatewayOutput::Decomposed {
            critical_model,
            critical_prompt,
    ...
    match adapter.complete(prompt).await {
        Ok(text) => {
            ...
            if !model.is_empty() && model != "auto" {
                resp.headers_mut().insert(
                    "x-hermess-routed-model",
                    axum::http::HeaderValue::from_str(model).unwrap_or_else(|_| axum::http::HeaderValue::from_static("")),
                );
            }
            if shg_triggered {
                resp.headers_mut().insert(
            ...
            if !route_reason.is_empty() {
                resp.headers_mut().insert(
                    "x-hermess-route-reason",
                    axum::http::HeaderValue::from_str(route_reason).unwrap_or_else(|_| axum::http::HeaderValue::from_static("")),
                );
            }
            resp
        }
...
async fn handle_stream(
    ...
    match adapter.complete_stream(prompt).await {
        Ok(stream) => {
            ...
            // Use axum response builder to include headers
            ...
            if !reason_owned.is_empty() {
                resp.headers_mut().insert(
                    "x-hermess-route-reason",
                    axum::http::HeaderValue::from_str(&reason_owned).unwrap_or_else(|_| axum::http::HeaderValue::from_static("")),
                );
            }
            resp
        }
