    ...
    async fn call(&self, args: serde_json::Value) -> anyhow::Result<ToolOutput> {
        ...
        match op {
            ...
            "create_node" => {
                ...
                let result = self.client.create_wiki_node(space_id, parent, title, content).await?;
                Ok(ToolOutput::text(serde_json::to_string_pretty(&result)?))
            }
            "update_node" => {
                let node_token = get_str(&args, "node_token")?;
                ...
                let content = args["content"].as_str().unwrap_or("");
                let result = self.client.update_wiki_node(node_token, title, content).await?;
                Ok(ToolOutput::text(serde_json::to_string_pretty(&result)?))
            }
            "search_wiki" => {
                ...
                let result = self.client.search_wiki(query, space_ids.as_deref()).await?;
                Ok(ToolOutput::text(serde_json::to_string_pretty(&result)?))
            }
...
impl FeishuDocsTool {
    pub fn new(client: Arc<FeishuClient>) -> Self { Self { client } }
}
    ...
    async fn call(&self, args: serde_json::Value) -> anyhow::Result<ToolOutput> {
        ...
        match op {
            ...
            "append_blocks" => {
                ...
                let result = self.client.append_document_blocks(doc_token, block_id, blocks).await?;
                Ok(ToolOutput::text(serde_json::to_string_pretty(&result)?))
            }
            "update_block" => {
                let doc_token = get_str(&args, "doc_token")?;
                ...
                let result = self.client.update_document_block(doc_token, block_id, update).await?;
                Ok(ToolOutput::text(serde_json::to_string_pretty(&result)?))
            }
            _ => Ok(ToolOutput::error(format!("未知操作: {op}"))),
        }
    ...
    async fn call(&self, args: serde_json::Value) -> anyhow::Result<ToolOutput> {
        ...
        match op {
            ...
            "upload_file" => {
                ...
                let ft = self.client.upload_drive_file(folder_token, file_name, data, mime_type).await?;
